...
while true; do
  ...
  print -u2 "FT 同步失败，${SYNC_RETRY_DELAY_S}s 后重试 (${attempt}/${SYNC_MAX_ATTEMPTS})"
...
for arg in "$@"; do
  ...
  case "$arg" in
    ...
    --date=*)
      ...
      ;;
    --login|--dry-run|--no-llm|--help|-h|--rebuild-outputs|--refresh-glossary|--repair-missing-translations)
      skip_repair=1
      ;;
  esac
done
if [[ "$REPAIR_AFTER_SYNC" != "0" && "$REPAIR_AFTER_SYNC" != "false" && "$REPAIR_AFTER_SYNC" != "False" && "$skip_repair" == "0" ]]; then
  "$PYTHON_BIN" sync_ft.py --repair-missing-translations "${repair_args[@]}"
fi
...
WSJ_OUTPUT_DIR=${WSJ_ARCHIVER_OUTPUT_DIR:-"$SCRIPT_DIR/../wsj_archiver_skill/output_results"}
FT_OUTPUT_DIR=${FT_ARCHIVER_OUTPUT_DIR:-"$SCRIPT_DIR/output_results"}
